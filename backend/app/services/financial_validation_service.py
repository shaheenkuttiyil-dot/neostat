...
def _filter_summary_rows(line_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def _check(name: str, formula: str, operands: Dict[str, Optional[float]],
    ...
    # A check passes if it's within the absolute tolerance OR the relative
    # tolerance (whichever is larger for this value) - an absolute-only
    # tolerance is meaningless for figures reported in thousands/crores,
    # where a genuine single-digit OCR misread produces a variance of a few
    # thousand that is proportionally tiny but dwarfs a small absolute unit.
            ...
            if a is not None:
                try:
                    ...
                    if row_reported != 0 and abs(row_calc - row_reported) / abs(row_reported) > 0.5:
                        # This row's own numbers don't reconcile with its own
                        # amount - almost certainly a column-alignment error
                        # (e.g. HSN code read as quantity), not a real total.
                        excluded_rows += 1
                        logger.info(
...
def validate_balance_sheet(fields: Dict[str, FieldValue]) -> ValidationResult:
    ...
    if liabilities is not None and equity is not None:
        # Statement reports liabilities and equity as separate totals.
        ...
        # Common in Indian Schedule III-format statements: a single combined
        # "Total Equity and Liabilities" figure rather than two separate
        # totals. Check it directly against total_assets instead of returning
        # NOT_APPLICABLE just because the two sub-totals weren't split out.
        checks.append(_check(
            "assets_equals_total_equity_and_liabilities",
            "total_equity_and_liabilities", {"total_equity_and_liabilities": combined},
            combined, assets,
        ))
    else:
        checks.append(_check(
